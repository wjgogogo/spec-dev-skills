...
本工具强制引入一条轻量级的规范链条：
**沟通需求（`sd-write-spec`）→ 制定实施任务（`sd-dev`）→ 隔离按阶段开发 → 对照规格审查校验（`sd-review`）**

## 核心技能

- `sd-write-spec`：通过提问交流，生成严谨的 EARS 需求文档并存入 `.claude/specs/`。
- `sd-dev`：严格根据需求文档，生成可执行的任务册 `.claude/tasks/`，向用户确认后完成开发，支持独立 worktree。
- `sd-review`：对照需求文件和任务文件，多维度对当前实现排查逻辑、规范和 Bug。
- `sd-proof`：对指定文档文件进行润色优化，确保符合中文文档书写规范。
